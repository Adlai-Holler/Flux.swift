...
final class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        let setupStore = SignalProducer<Void, NSError>.attempt {
            do {
                self.coreDataStack = try CoreDataStack(model: NSManagedObjectModel.mergedModelFromBundles(nil)!, persistentStoreURL: AppDelegate.dataStoreURL)
                return Result(value: ())
        ...
        let todoStore = TodoStore(managedObjectContext: coreDataStack!.managedObjectContext)
