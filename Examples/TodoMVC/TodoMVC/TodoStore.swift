...
final class TodoStore: Store<TodoAction> {
    ...
    init(managedObjectContext: NSManagedObjectContext) {
		...
		super.init(dispatcher: appDispatcher)
    }
    ...
    func getAll() -> [TodoItem] {
		...
		fr.predicate = NSPredicate(format: "softDeleted == false")
	...
	override func onDispatch(action: TodoAction) {
        switch action {
        ...
        case let .APICreateFailed(temporaryID, _):
            managedObjectContext.performBlock {
				var doomedItemTitle: String?
                let changeOrNil = try? self.managedObjectContext.doWriteTransaction {
                    for item in self.getAll() where item.id == temporaryID {
						doomedItemTitle = item.title
                ...
                if let change = changeOrNil {
                    self.changeObserver.sendNext(.Change(modelChange: change, errorMessage: "Failed to create Todo item '\(doomedItemTitle ?? "(Untitled)")'."))
                }
			}
		}
    }
